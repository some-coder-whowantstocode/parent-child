...
export const GET = errorHandler(async function GET(req: NextApiRequest) {
  ...
  const page = Math.floor(
    Math.max(Number(requrl.searchParams.get("page")) || 1, 1)
  );
  ...
  const userdata = await User.aggregate([
    ...
    {
      $project: {
        Connections: { $slice: ["$Connections", skip, limit] },
        isdeleted: 1,
        isverified: 1,
      },
    },
