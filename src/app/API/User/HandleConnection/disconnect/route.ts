...
export async function POST(req: NextApiRequest) {
    try {
        ...
        await dbconnect();
        const user = await User.updateOne({_id:identifier},{$pull:{Connections:{id:decoded.id}}});

        if(user.modifiedCount == 0){
            return NextResponse.json({err:'No such user exists'},{status:400})
        }

        const reciever = await User.updateOne({_id:decoded.id},{$pull:{Connections:{id:identifier}}});
